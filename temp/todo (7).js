            ...
            $(value).sortable({
                ...
                change: function(event, ui) {
                    if (ui.sender == null) {
                        ...
                        if (ui.item.data("points") <= (taskPoints + itemsAbove) && runOnce == false && position >= positionTwo) {
                            runOnce = true;
                            itemsAbove = 0;
                            console.log("runonce " + (taskPoints + itemsAbove));
                        } else {
                            runOnce = false;
                            console.log("false");
                        }

                        if (ui.item.data("points") >= taskPoints || runOnce == true) {

                            console.log("points >= taskpoints");

                            if (placeholder.parent().attr('id') == "pending" && currentArea == "inProgress" && allTaskPoints > budget && position >= positionTwo || runOnce == true) {
                            console.log("alltaskpoints > budget");
                            taskItems = $();
                            taskPoints = 0;
